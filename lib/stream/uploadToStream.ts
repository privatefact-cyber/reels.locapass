...
export async function uploadToStream(file: File, onProgress?: (percent: number) => void) {
  ...
  await new Promise<void>((resolve, reject) => {
    const upload = new tus.Upload(file, { uploadUrl: uploadURL, retryDelays: [0, 1000, 3000, 5000], chunkSize: 5 * 1024 * 1024, metadata: { filename: file.name, filetype: file.type }, onError: (cause) => reject(cause), onProgress: (sent, total) => onProgress?.(Math.round(sent / total * 100)), onSuccess: () => resolve() });
    upload.start();
  });
  return uid;
}
