  ...
  return (
    ...
    >
      ...
      <span className="pointer-events-none absolute inset-x-0 bottom-0 px-2 pb-1.5 pt-4">
        <span className="block truncate text-[11px] font-semibold text-white drop-shadow">{event.title}</span>
        <span className="block truncate text-[10px] text-white/70">{event.shopName}</span>
      </span>
    </button>
  );
});
