  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">
        発行後、ポータルの画面でポータル管理者(portal_admin)を招待できます。
      </p>

      <form ref={formRef} action={formAction} className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="text-sm font-medium text-slate-700">
            ポータル名 <span className="text-red-500">*</span>
          </span>
          <input name="name" required placeholder="例: 水戸まちあるきポータル" className={INPUT} />
        </label>
        <label className="block">
          <span className="text-sm font-medium text-slate-700">
        ...
        <label className="block">
          ...
          <input name="tagline" placeholder="例: 水戸の街歩きをもっと楽しく" className={INPUT} />
        </label>
        <label className="block">
          <span className="text-sm font-medium text-slate-700">ポータルのURL</span>
          <input name="home_url" type="url" placeholder="https://" className={INPUT} />
        </label>
