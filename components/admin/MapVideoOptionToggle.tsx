...
import { setShopMapVideo } from "@/app/admin/(console)/actions";

export function MapVideoOptionToggle({ shopId, enabled }: { shopId: string; enabled: boolean }) {
  const [pending, startTransition] = useTransition();

  function handleClick() {
    startTransition(async () => {
      await setShopMapVideo(shopId, !enabled);
    });
  }
