...
import { SingleImageDropzone, EventGalleryDropzone } from "@/components/dashboard/EventImageDropzone";
import { TimeOfDaySelect } from "@/components/dashboard/TimeOfDaySelect";
...
export function StaffMypageClient({
  staffId,
  shopId,
  shopName,
  initialName,
  ...
  async function handleAvatarCropped(blob: Blob) {
    ...
    const { error: rpcError } = await supabase.rpc("update_own_staff_profile", {
      p_name: name,
      p_bio: bio,
  ...
  async function handleSubmitReel(e: React.FormEvent) {
    ...
    const [{ error: uploadError }, previewUrl] = await Promise.all([
      ...
      isVideo ? uploadReelPreview(supabase, staffId, file) : Promise.resolve(null),
    ]);
    if (uploadError) {
      setUploading(false);
  ...
  async function handleDeleteEvent(eventId: string) {
    const supabase = createClient();
    const { error: deleteError } = await supabase.from("shop_events").delete().eq("id", eventId);
  ...
  async function handleReplyInquiry(inquiryId: string) {
    ...
    const supabase = createClient();
    const { error: insertError } = await supabase.from("shop_inquiry_messages").insert({
