...
export function EventExpandedCard({ event }: { event: PortalEvent }) {
  ...
  return (
    <div data-surface="media" className="relative aspect-[9/16] w-full transform-gpu will-change-transform md:h-full md:w-auto">
      <div className="absolute inset-0 overflow-hidden rounded-2xl bg-tone-950">
        ...
        <div className="absolute inset-x-3 top-3 flex flex-wrap gap-2">
          {event.area && (
            <span className="rounded-full bg-black/50 px-2.5 py-1 text-[11px] font-semibold text-main backdrop-blur-sm">
              📍 {event.area}
            </span>
          )}
          {event.genre && (
      ...
      <div className="pointer-events-auto absolute bottom-4 left-3 right-16 space-y-2">
        <h2 className="font-display text-xl font-bold leading-tight tracking-wide text-main drop-shadow-lg">
          {event.title}
        </h2>
        <Link
          href={shopUrl}
        ...
        <Link href={shopUrl} className="block w-fit text-main drop-shadow">
          <p className="text-sm font-semibold">{event.shopName}</p>
          {dates && <p className="text-xs text-main/80">{dates}</p>}
        </Link>
      </div>
    </div>
  );
