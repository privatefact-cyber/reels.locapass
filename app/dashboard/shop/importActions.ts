...
} from "@/lib/shop/importFromWebsite";
...
export async function previewImportAction(url: string): Promise<PreviewImportResult> {
  const shop = await requireCurrentShop();
  if (!shop) return { ok: false, error: "所属店舗が見つかりません" };
  try {
...
export async function applyImportAction(input: ApplyImportInput): Promise<ApplyImportResult> {
  ...
  const rehost = async (src: string, label: string) => {
    ...
    const path = `${shop.id}/imported-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${image.ext}`;
    const { error } = await supabase.storage
      .from("shop-events")
      .upload(path, image.bytes, { contentType: image.contentType });
    if (error) throw new ImportError(`${label}の保存に失敗しました: ${error.message}`);
    return supabase.storage.from("shop-events").getPublicUrl(path).data.publicUrl;
  };

  try {
    if (input.coverImageUrl) {
      patch.cover_image_url = await rehost(input.coverImageUrl, "メイン画像");
      applied.push("メイン画像");
    }
  ...
  if (Object.keys(patch).length > 0) {
    const { data, error } = await supabase.from("shops").update(patch).eq("id", shop.id).select("id");
