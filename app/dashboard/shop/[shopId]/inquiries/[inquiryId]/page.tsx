...
/**
 * LUXELA本家のお問い合わせ詳細(app/dashboard/inquiries/[inquiryId]/page.tsx)と同じ画面。
 * locapass_shop_inquiries / locapass_shop_inquiry_messages につないである。他店舗の問い合わせIDは404。
 */
export default async function LocapassShopInquiryThreadPage({
  params,
}: {
  params: Promise<{ shopId: string; inquiryId: string }>;
}) {
  const { shopId, inquiryId } = await params;
  ...
  const boundReply = replyToInquiry.bind(null, shop.id, inquiryId);
