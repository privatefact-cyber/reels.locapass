...
import { addStaffMember, deleteStaffMember, regenerateStaffLoginToken } from "../not-connected";

/**
 * LUXELA本家のスタッフ管理画面(app/dashboard/staff/page.tsx)と同じ画面。
 * locapassにはスタッフ個人アカウントの受け皿テーブルがまだ無いため、一覧は空で、
 * 追加・発行などの操作は「未接続」を返す(LUXELAのshop_staff_membersは読まない)。
 */
export default async function LocapassShopStaffPage({
  ...
  const staffMembers: {
    ...
    user_id: string | null;
    staff_login_tokens: { token: string } | { token: string }[] | null;
  }[] = [];
